...
window.SceneManager = class SceneManager {
    ...
    restore(data, replace = true) {
        ...
        try {
            ...
            for (const item of data.instances) {
                if (!item || !item.name || !this.assetManager.hasTemplate(item.name)) { skipped++; continue; }
            ...
            if (skipped > 0)
                console.warn(`[SceneManager] ${skipped} asset(s) ignoré(s) — template introuvable (schem pas encore chargé ?).`);
            return count;
        } finally {
            this._restoring = false;
        }
    ...
    loadFromLocal() {
        try {
            ...
            if (!raw) return 0;
            return this.restore(JSON.parse(raw), true);
        } catch (e) {
    ...
    importFile(file) {
        return new Promise((resolve, reject) => {
            ...
            reader.onload = () => {
                try { resolve(this.restore(JSON.parse(reader.result), true)); }
                catch (e) { reject(e); }
            };
